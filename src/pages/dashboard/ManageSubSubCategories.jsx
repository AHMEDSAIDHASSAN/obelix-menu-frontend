...
const EMPTY = { name: '', nameAr: '', subCategory: '', order: 0, isActive: true };

export default function ManageSubSubCategories() {
  ...
  const load = () =>
    Promise.all([api.get('/subsubcategories/all'), api.get('/subcategories/all')]).then(([s, sc]) => {
      setItems(s.data); setSubCategories(sc.data);
    });
  ...
  const openEdit = (item) => {
    setForm({ name: item.name, nameAr: item.nameAr || '', subCategory: item.subCategory?._id || '', order: item.order, isActive: item.isActive });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) await api.put(`/subsubcategories/${editId}`, form);
      else await api.post('/subsubcategories', form);
  ...
  const handleDelete = async (id) => {
    ...
    await api.delete(`/subsubcategories/${id}`); load();
  };

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-black text-gray-900">الفئات الفرعية الثانية</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-base font-bold text-gray-900">{editId ? 'تعديل' : 'إضافة فئة فرعية ثانية'}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">الفئة الفرعية *</label>
                <select required value={form.subCategory} onChange={(e) => setForm({ ...form, subCategory: e.target.value })}
                  ...
                  {subCategories.map((s) => (
                    <option key={s._id} value={s._id}>
                      {s.category?.name ? `${s.category.name} ← ` : ''}{s.name}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex gap-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="px-5 bg-white border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl text-sm hover:bg-gray-50">إلغاء</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {items.length === 0 ? (
          <div className="text-center py-16 text-gray-400 text-sm">لا توجد فئات فرعية ثانية بعد</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="px-5 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">الاسم</th>
                <th className="px-5 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">الفئة الفرعية</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-sm text-gray-500 bg-gray-100 px-2.5 py-1 rounded-lg">{item.subCategory?.name || '—'}</span>
                  </td>
